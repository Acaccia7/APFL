...
class RBM(nn.Module):
    ...
    def __init__(self,
        ...
        self.W = nn.Parameter((torch.randn(n_visible, n_hidden) * 0.01).to(self.device))
        self.vb = nn.Parameter((torch.zeros(self.n_visible)).to(self.device))
        self.hb = nn.Parameter((torch.zeros(self.n_hidden)).to(self.device))
