...
from utils.DisruptingModels import DisruptingModels

temp = sys.stdout
f = open('log/turnover50', 'w')
sys.stdout = f
...
def run(args, config):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "cnn":
            ...
            mymodel = torch.load(os.path.join(MODEL_DIR, f'{config["name"]}_seg.pt'))
            args.model = mymodel['model_state_dict']
...
if __name__ == "__main__":
    ...
    with open('../deepBeliefNetwork.json', 'r') as read_file:
