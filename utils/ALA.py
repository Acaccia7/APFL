...
class ALA:
    ...
    def adaptive_local_aggregation(self,
        ...
        num_subset_samples = int(num_samples * self.rand_percent // 100)

        
        train_subset, _ = random_split(self.train_loader.dataset,
                                       [num_subset_samples, num_samples - num_subset_samples])

        
        rand_loader = DataLoader(train_subset, batch_size=self.batch_size, shuffle=False, num_workers=4,
                                 pin_memory=True)
        ...
        params_g = []
        params = []
        for m in global_model.models:
            params_g += list(m.parameters())
        for m in local_model.models:
        ...
        weight_t = copy.deepcopy(self.weights)
        ...
        while True:
            ...
            for x, y in rand_loader:
                ...
                optimizer.zero_grad()
                output = model_t(x)
                loss_value = self.loss(output, y.squeeze())  # modify according to the local objective
                loss_value.backward()

                avg_los.append(loss_value.item())
            ...
            losses.append(sum(avg_los) / len(avg_los))
            cnt += 1
            print('Client:', self.cid, 'The', cnt, 'update', sep='')
